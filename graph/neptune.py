...
────────────────────────────────────────────
  ...
  iam_role_arn    str   IAM role ARN Neptune uses to read from S3
...
class NeptuneGraphTarget(GraphTarget):
    ...
    def __init__(self, config: GraphTargetConfig) -> None:
        ...
        self._node_buf: io.BytesIO | None = None
        self._edge_buf: io.BytesIO | None = None
        self._node_writer = None
        self._edge_writer = None
        self._tag_rows: List[List[str]] = []
    ...
    def _gremlin(self, query: str) -> None:
        resp = requests.post(self._gremlin_url, json={"gremlin": query}, timeout=60)
    ...
    def begin_bulk(self) -> None:
        ...
        self._tag_rows  = []
        node_wrap = io.TextIOWrapper(self._node_buf, encoding="utf-8", newline="", write_through=True)
        edge_wrap = io.TextIOWrapper(self._edge_buf, encoding="utf-8", newline="", write_through=True)
        self._node_writer = csv.writer(node_wrap)
        self._edge_writer = csv.writer(edge_wrap)
        # headers are written per-mapping by write_bulk_node/edge via BulkRecord
    ...
    def commit_bulk(self) -> str:
        ...
        resp = requests.post(self._loader_url, json=payload, timeout=30)
        resp.raise_for_status()
        ...
        while time.monotonic() < deadline:
            ...
            r.raise_for_status()
            status = r.json().get("payload", {}).get("overallStatus", {}).get("status", "UNKNOWN")
            ...
            raise TimeoutError(f"Neptune bulk load {job_id} timed out after {self._poll_timeout}s")
